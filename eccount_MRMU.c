 ...
 *
 * Usage: ./eccount_MRMU pperf nreaders nwriters affinity_config_file duration (ms)
 *
...
/* read() and update() are the function names used in ECCount paper */
#define update inc_count 
#define read read_count 

void update(long thread_num)
{
...
int cleanup_old_snapshots(struct snapshot * sp)
{
	...
	// Locate the last and the second to last snapshots.
	...
	/* Now, we are ready to free a series of snapshots in between repp and rep_anchor.
	 * We choice a Read-Copy-Update based synchronization strategy.
	 * The following code corresponds to alorithm free_snapshots_RCU in ECCount paper.
	 * The only difference is that the code shown here can free a series of snapshots, 
	 * instead of only two of them, in each iteration.*/
...
unsigned long read(int me)
{
	...
	/* Now, new_snapshot points to the latest snapshot which updaters were incrementing.
	 * We then insert this snapshot into global linked list, global_old_snapshots.
	 * The following code corresponds to helper function insert() in ECCount paper.      */
restart:
	smp_mb();
	...
	if ( latest_snapshot_old_value->timestamp < latest_snapshot->timestamp ) {
		// This is the common case; snapshot will be inserted at head.
		...
		// then insert the latest snapshot to build an in-order linked list
